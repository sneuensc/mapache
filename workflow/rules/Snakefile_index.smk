...
rule check_chromosomes:
    ...
    output:
        touch('results/00_reference/{id_genome}/{id_genome}.ok')
    params:
        femaleChr   = lambda wildcards: get_param3('genome',wildcards.id_genome,'femaleChr','X'),
        maleChr     = lambda wildcards: get_param3('genome',wildcards.id_genome,'maleChr','Y'),
        mtChr       = lambda wildcards: get_param3('genome',wildcards.id_genome,'mtChr','MT'),
        autosomeChr = lambda wildcards: eval_to_list(get_param3('genome',wildcards.id_genome,'autosomeChr',''))
    log:
        'results/00_reference/{id_genome}/{id_genome}.ok.log'
    message: "--- TESTING CHROMOSOME NAMES"
    run:
        import numpy as np
        ...
        if params.femaleChr not in allChr:
            ...
            print(f"WARNING: In parameter 'genome:{wildcards.id_genome}:femaleChr' the argument '{params.mtChr}' is unknown, assuming no female chromosome!")
        
        if params.maleChr not in allChr:
            set_param3('genome',wildcards.genome,'maleChr','')
            print(f"WARNING: In parameter 'genome:{wildcards.id_genome}:maleChr' the argument '{params.mtChr}' is unknown, assuming no male chromosome!")
        
        if params.mtChr not in allChr:
            set_param3('genome',wildcards.genome,'mtChr','')
            print(f"WARNING: In parameter 'genome:{wildcards.id_genome}:mtChr' the argument '{params.mtChr}' is unknown, assuming no MT chromosome!")
        
        if params.autosomeChr == '':
            autosomeChr = np.intersect1d(allChr, [params.femaleChr, params.maleChr, params.mtChr])
            set_param3('genome',wildcards.id_genome,'autosomeChr',list_to_csv(autosomeChr))
        else:
            ...
            if len(unknown) > 0:
                ...
                set_param3('genome',wildcards.id_genome,'autosomeChr',list_to_csv(autosomeChr))
                if len(unknown) == 1:
                    print(f"WARNING: In parameter 'genome:{wildcards.id_genome}:autosomeChr' the name {unknown} is unknown, ignoring it!")
                else:
                    print(f"WARNING: In parameter 'genome:{wildcards.id_genome}:autosomeChr' the names {unknown} are unknown, ignoring them!")
