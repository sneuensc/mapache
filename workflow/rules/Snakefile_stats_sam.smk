...
rule multiqc_fastqc:
    ...
    conda:
        "../envs/multiqc.yaml"
    message:
        "--- MULTIQC fastqc_{wildcards.group}: {input}"
    shell:
        """
...
rule multiqc_flagstat:
    ...
    output:
        html="{folder}/multiqc_flagstat_{group}.{GENOME}.html",
        txt="{folder}/multiqc_flagstat_{group}.{GENOME}_data/multiqc_samtools_flagstat.txt",
    resources:
        memory=lambda wildcards, attempt: get_memory_alloc("multiqc_mem", attempt, 2),
        runtime=lambda wildcards, attempt: get_runtime_alloc("multiqc_time", attempt, 1),
    log:
        "{folder}/multiqc_flagstat_{group}.{GENOME}.log",
    conda:
        "../envs/multiqc.yaml"
    message:
        "--- MULTIQC flagstat_{wildcards.group}: {input}"
    shell:
        """
        multiqc -n $(basename {output.html}) -f -d -o $(dirname {output.html}) {input} \
        -b \"Merged flagstat output of {wildcards.group} libraries and genome {wildcards.GENOME}\" 2> {log}
        """
...
rule write_summary_stats_fastq:
    ...
    input:
        ...
        final="{folder}/01_multiqc/multiqc_flagstat_final.{GENOME}_data/multiqc_samtools_flagstat.txt",
    output:
        fastq_stats=report(
            "{folder}/02_summary/fastq_stats.{GENOME}.csv",
            caption="../report/fastq_stats.rst",
            category="Mapping statistics table",
        ),
    log:
        "{folder}/02_summary/fastq_stats.{GENOME}.log",
    message:
        "--- WRITE FASTQ SUMMARY STATISTICS OF {wildcards.GENOME}"
    run:
        import pandas as pd
        import numpy as np
        ...
        def read_flagstat(file, extract, name):
            d1 = pd.read_csv(file, sep="\t").rename({extract: name}, axis=1)[
                ["Sample", name]
            ]
            d1[name] = d1[name].astype("int64")
            d2 = pd.DataFrame(
                d1["Sample"].str.split("|").tolist(),
                columns=["type", "type", "type", "type", "SM", "LB", "ID"],
            )
            ...
            d2["ID"] = d2["ID"].map(
                lambda x: x.rstrip(".{}_flagstat".format(wildcards.GENOME))
            )
        ...
        ## sample file
        db_fastq = (
            pd.read_csv(sample_file, sep=delim)[["SM", "LB", "ID"]]
        ...
        db_fastq["trim_prop"] = np.where(
            db_fastq["reads_raw"].ne(0),
            db_fastq["reads_trim"].astype(float) / db_fastq["reads_raw"],
            np.NaN,
        )
        db_fastq["endo_prop"] = np.where(
            db_fastq["reads_raw"].ne(0),
            db_fastq["mapping"].astype(float) / db_fastq["reads_raw"],
            np.NaN,
        )
        ...
        ].round({"reads_trim": 0, "trim_prop": 4, "mapping": 0, "endo_prop": 4}).to_csv(
            output[0], index=None, header=True
        )
...
rule write_summary_stats_library:
    ...
    """
    input:
        fastq="results/01_fastq/05_stats/02_summary/fastq_stats.{GENOME}.csv",
        final="results/02_library/04_stats/01_multiqc/multiqc_flagstat_final.{GENOME}_data/multiqc_samtools_flagstat.txt",
    output:
        fastq_stats=report(
            "results/02_library/04_stats/02_summary/library_stats.{GENOME}.csv",
            caption="../report/library_stats.rst",
            category="Mapping statistics table",
        ),
    log:
        "results/02_library/04_stats/02_summary/library_stats.{GENOME}.log",
    message:
        "--- WRITE LIBRARY SUMMARY STATISTICS OF {wildcards.GENOME}"
    run:
        import pandas as pd
        import numpy as np
        ...
        def read_flagstat(file, extract, name):
            ...
            d2 = pd.DataFrame(
                d1["Sample"].str.split("|").tolist(),
                columns=["type", "type", "type", "type", "SM", "LB"],
            )
            d2["SM"] = d2["SM"].str.strip()
            d2["LB"] = d2["LB"].str.strip()
            d2["LB"] = d2["LB"].map(
        ...
        db_library = (
            db_fastq.groupby(["SM", "LB"])[["reads_raw", "reads_trim", "mapping"]]
        ...
        db_library["trim_prop"] = np.where(
            db_library["reads_raw"].ne(0),
            db_library["reads_trim"].astype(float) / db_library["reads_raw"],
            np.NaN,
        )
        db_library["endo_prop"] = np.where(
            db_library["reads_raw"].ne(0),
            db_library["mapping"].astype(float) / db_library["reads_raw"],
            np.NaN,
        )
        db_library["endo_final_prop"] = np.where(
            db_library["reads_raw"].ne(0),
            db_library["mapping_final"].astype(float) / db_library["reads_raw"],
            np.NaN,
        )
        db_library["duplicates_prop"] = np.where(
            db_library["mapping"].ne(0),
            db_library["duplicates"].astype(float) / db_library["mapping"],
            np.NaN,
        )
...
rule write_summary_stats_sample:
    ...
    """
    input:
        library="results/02_library/04_stats/02_summary/library_stats.{GENOME}.csv",
    output:
        sample_stats=report(
            "results/03_sample/04_stats/01_summary/sample_stats.{GENOME}.csv",
            caption="../report/sample_stats.rst",
            category="Mapping statistics table",
        ),
    log:
        "results/03_sample/04_stats/01_summary/sample_stats.{GENOME}.log",
    message:
        "--- WRITE SAMPLE SUMMARY STATISTICS OF {wildcards.GENOME}"
    run:
        import pandas as pd
        import numpy as np
        ...
        db_sample = (
            db_library.groupby("SM")[
                ["reads_raw", "reads_trim", "mapping", "mapping_final", "duplicates"]
            ]
            .apply(lambda x: x.sum(skipna=False))
            .reset_index()
        )
        db_sample["trim_prop"] = np.where(
            db_sample["reads_raw"].ne(0),
            db_sample["reads_trim"].astype(float) / db_sample["reads_raw"],
            np.NaN,
        )
        db_sample["endo_prop"] = np.where(
            db_sample["reads_raw"].ne(0),
            db_sample["mapping"].astype(float) / db_sample["reads_raw"],
            np.NaN,
        )
        db_sample["endo_final_prop"] = np.where(
            db_sample["reads_raw"].ne(0),
            db_sample["mapping_final"].astype(float) / db_sample["reads_raw"],
            np.NaN,
        )
        db_sample["duplicates_prop"] = np.where(
            db_sample["mapping"].ne(0),
            db_sample["duplicates"].astype(float) / db_sample["mapping"],
            np.NaN,
        )
...
rule plot_summary_statistics:
    ...
    """
    input:
        fastq_stats="results/01_fastq/05_stats/02_summary/fastq_stats.{GENOME}.csv",
        library_stats="results/02_library/04_stats/02_summary/library_stats.{GENOME}.csv",
    ...
    log:
        "results/03_sample/04_stats/01_summary/plot_summary_statistics_{GENOME}.log",
    conda:
        "../envs/r.yaml"
    envmodules:
        module_r,
    message:
        "--- PLOT SUMMARY STATISTICS OF {wildcards.GENOME}"
    script:
...
rule depth_compute:
    ...
    """
    input:
        "{folder}/{file}.{GENOME}.bam",
    output:
        "{folder}/{file}.{GENOME}_depth.txt",
    resources:
    ...
    log:
        "{folder}/{file}.{GENOME}.log",
    threads: 1
    message:
        "--- COMPUTE DEPTH OF {input}"
    shell:
...
rule concat_depth_statistics:
    ...
    output:
        report(
            "{folder}/depth_stats_{GENOME}.csv",
            caption="../report/sample_depth.rst",
            category="Mapping statistics table",
        ),
    threads: 1
    log:
        "{folder}/depth_stats_{GENOME}.log",
    message:
        "--- COMBINE DETH STATISTICS TO {output} ---"
    script:
